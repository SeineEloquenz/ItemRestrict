...
public class RestrictedItemsHandler {
    ...
    public MaterialData isBanned(ActionType actionType, Player player, Material type, short data, Location location) {
        ...
        if (player != null && player.hasPermission("ItemRestrict.admin") || player != null && player.hasPermission("ItemRestrict.bypass"))
            return null;
        ...
        MaterialData bannedInfo = collectionToSearch.Contains(new MaterialData(type, data, null, null));
        if (bannedInfo != null) {
            if (player == null) return bannedInfo;
            if (player.hasPermission("ItemRestrict.bypass." + type + ".*.*")) return null;
            if (player.hasPermission("ItemRestrict.bypass." + type + ".*." + permissionNode)) return null;
            if (player.hasPermission("ItemRestrict.bypass." + type + "." + data + "." + permissionNode)) return null;
            if (player.hasPermission("ItemRestrict.bypass." + type + "." + data + ".*")) return null;
