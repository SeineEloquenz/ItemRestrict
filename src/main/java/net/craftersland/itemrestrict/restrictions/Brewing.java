...
public class Brewing implements Listener {
    ...
    private void oldFunction(final BrewEvent event) {
        ...
        if (event.getContents().getItem(0) != null) {
            potionSlot0 = new ItemStack(event.getContents().getItem(0).getType(), 1, event.getContents().getItem(0).getDurability());
        }
        if (event.getContents().getItem(1) != null) {
            potionSlot1 = new ItemStack(event.getContents().getItem(1).getType(), 1, event.getContents().getItem(1).getDurability());
        }
        if (event.getContents().getItem(2) != null) {
            potionSlot2 = new ItemStack(event.getContents().getItem(2).getType(), 1, event.getContents().getItem(2).getDurability());
        }
        ...
        //Check the brewed potions for banned potions. Delayed task to check the potions after brewing.
        Bukkit.getScheduler().scheduleSyncDelayedTask(ir, () -> {
            Player player = null;
            boolean restricted = false;
            boolean ingConsumed = false;
            String reason = "";
            ...
            //Check slot 0 for banned items
            if (item0 != null) {
                MaterialData bannedInfo = ir.getRestrictedItemsHandler().isBanned(ActionType.Brewing, player, item0.getType(), item0.getData().getData(), event.getBlock().getLocation());
            ...
            //Check slot 1 for banned items
            if (item1 != null) {
                MaterialData bannedInfo = ir.getRestrictedItemsHandler().isBanned(ActionType.Brewing, player, item1.getType(), item1.getData().getData(), event.getBlock().getLocation());
            ...
            //Check slot 2 for banned items
            if (item2 != null) {
                MaterialData bannedInfo = ir.getRestrictedItemsHandler().isBanned(ActionType.Brewing, player, item2.getType(), item2.getData().getData(), event.getBlock().getLocation());
